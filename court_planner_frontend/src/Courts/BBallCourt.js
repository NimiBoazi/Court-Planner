...
const BBallCourt = ({
  ...
  const confirmedBookingDate = (date, start, end) => {
    ...
    setBookingDate(
      `Your booking for ${dayOfWeek}, ${day}/${month}/${year} at ${start}-${end} has been confirmed.`
    );
  };
  ...
  return (
    <div className="court-container">
      ...
      {showTimeGrid && (
        <TimeGrid
          user={user}
          closeTimeGrid={closeTimeGrid}
          openingTime={openingTime}
          closingTime={closingTime}
          locationID={locationID}
          courtNumber={courtNumber}
          maxCap={maxCap}
          confirmedBookingDate={confirmedBookingDate}
        />
      )}
      {showConfirmedBooking && (
        <ConfirmedBookingPage
          bookingDate={bookingDate}
          closeConfirmedBooking={closeConfirmedBooking}
        />
      )}
      {showViewSchedule && (
        <ViewSchedule
